...
export class PocController {
  ...
  async createPoc(
    ...
    // Ensure the event code matches the path parameter
    createPocDto.eventCode = eventId;
  ...
  @Roles(UserRole.POC)
  @ApiOperation({
    summary: 'Get all points of check-in managed by the current user',
  })
  @ApiResponse({
    status: HttpStatus.OK,
    description: 'Returns all points of check-in managed by the user',
  })
  async getUserPocs(@CurrentUser() user: JwtPayload) {
    return this.pocService.findAllByUsername(user.username);
  }
}
